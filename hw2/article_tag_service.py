...
class ArticleTagService:
    ...
    def process_single_article(self, article_id: int) -> Dict[str, Any]:
        ...
        try:
            # Get article from GraphQL
            data = self.get_articles_from_graphql(1, 100)  # Get more articles to find the specific one
            ...
            # Extract keywords
            keywords = self.extract_keywords(target_article["text"])
            
            # Save tags to database
            if self.save_tags_to_db(article_id, keywords):
                return {
                    "article_id": article_id,
                    "article_name": target_article["name"],
                    "tags": keywords,
                    "success": True
                }
            else:
                return {"error": "Failed to save tags to database"}
